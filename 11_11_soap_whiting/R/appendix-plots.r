...
load("run/app-output.RData")
load("data/land.rdata")
load("data/limits.rdata")

do.one.plot <- expression(
{
...
p.dat $ ssb <- NA
sy <- ssb.years[ssb.years %in% years]
p.dat $ ssb[p.dat $ year %in% sy] <- unlist(ssb.fits[paste(sy)])
fssb <- function(ssb) ssb / max(p.dat $ ssb, na.rm = TRUE)
ifssb <- function(at) at * max(p.dat $ ssb, na.rm = TRUE)
p.dat $ s.ssb <- fssb(p.dat $ ssb)

p.dat $ rec <- NA
ry <- rec.years[rec.years %in% (years + 1)]
p.dat $ rec[p.dat $ year %in% (ry - 1)] <- unlist(rec.fits[paste(ry)])
...
levelplot(s.rec ~ x * y | factor(year), data = subset(p.dat, year %in% years[1:2]),
		  ...
		  xlab = "", ylab = "", layout = c(5,1), strip = FALSE,
		  between = between, 
...
scale <- 2.4
iheight <- 7.5 / ratio # inches
iwidth <- 7.5 # inches
dpi <- 300

png(file=paste0("fig/", fname, ".png"), width = iwidth*scale, height = iheight*scale, res = round(dpi/scale), units = "in")
  
# the figure
olap <- .06

plot(legend.p, position = c(0.18, .66 - olap, 1, 1), more = TRUE)
plot(ssb.p, position = c(0.18, .33 - olap/2, 1, .66 + olap/2), more = TRUE)
plot(rec.p, position = c(0.18, 0, 1, .33 + olap), more = FALSE)	  
dev.off()

str <- paste0("convert ", "fig/", fname, ".png", " fig/", fname, ".tiff")
system(str)
...
for (i in seq(1987, 2011, by = 5)) {
  years <- i + 0:4
  fname <- paste0("app-plot-",i)
  ratio <- 1.5
  eval(do.one.plot)
}
